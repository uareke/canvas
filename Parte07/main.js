...
function main() {
    try {
        ...
        // --- NOVA FUNÇÃO: RENDERIZAR O PAINEL DE CAMADAS ---
        function renderLayerPanel() {
            layerList.innerHTML = ''; // Limpa a lista atual

            // Loop invertido para que a camada mais nova (e de cima) apareça no topo da lista
            for (let i = sceneObjects.length - 1; i >= 0; i--) {
                const obj = sceneObjects[i];
                const li = document.createElement('li');
                li.className = 'layer-item';
                li.textContent = `Camada ${i}`;// Usamos o nome do arquivo guardado no objeto
                li.dataset.index = i; // Guarda o índice do objeto no atributo 'data-index'

                if (i === selectedIndex) {
                    li.classList.add('active'); // Adiciona a classe 'active' se for o item selecionado
                }
        ...
        fileInput.addEventListener('change', (event) => {
            // Permite carregar múltiplos arquivos de uma vez
        ...
        canvas.addEventListener('mousedown', (event) => {
            ...
            redrawCanvas();
            //NOVO
            renderLayerPanel(); // SINCRONIZA o painel com a seleção feita no canvas
            //FIM NOVO
        });
        ...
        //NOVO EVENTO
         // CLIQUE NO PAINEL DE CAMADAS (usando delegação de eventos)
