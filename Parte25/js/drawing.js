...
export function redrawCanvas() {
    ...
    // --- PARTE NOVA: DESENHANDO AS RÉGUAS ---
    // Esta função (que criaremos a seguir) desenha as réguas e suas marcações
    drawRulers();
    ...
    // Move a "origem" do desenho para depois das réguas
    ...
    // Desenha as guias por baixo dos objetos
    drawGuides();

    state.sceneObjects.forEach(obj => {
        if (obj.isVisible) {
            const { x, y, width, height, rotation, shadow } = obj;
            ctx.save();
