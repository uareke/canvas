...
function resizeImage(obj, mouseX, mouseY) {
            ...
            const { x, y, width, height, rotation } = obj; // Precisamos da rotação para a lógica avançada
            
            // Lógica de redimensionamento para objetos rotacionados é EXTREMAMENTE complexa.
            // A lógica abaixo é uma simplificação que funciona bem para objetos não rotacionados
            // e de forma "ok" para objetos rotacionados, mas não é perfeita.
            // Implementar redimensionamento perfeito em objetos rotacionados é um tópico de Nível 3.
            
            const localMouseX = mouseX - (x + width / 2);
            const localMouseY = mouseY - (y + height / 2);
            ...
            // Esta lógica simplificada não vai funcionar bem com rotação. Vamos manter a antiga por enquanto.
            switch (activeHandle) {
                case 'top-left': newWidth = Math.max(x + width - mouseX, minSize); newHeight = Math.max(y + height - mouseY, minSize); newX = x + width - newWidth; newY = y + height - newHeight; break;
                case 'top-center': newHeight = Math.max(y + height - mouseY, minSize); newY = y + height - newHeight; break;
                case 'top-right': newWidth = Math.max(mouseX - x, minSize); newHeight = Math.max(y + height - mouseY, minSize); newY = y + height - newHeight; break;
                case 'middle-left': newWidth = Math.max(x + width - mouseX, minSize); newX = x + width - newWidth; break;
                case 'middle-right': newWidth = Math.max(mouseX - x, minSize); break;
                case 'bottom-left': newWidth = Math.max(x + width - mouseX, minSize); newHeight = Math.max(mouseY - y, minSize); newX = x + width - newWidth; break;
                case 'bottom-center': newHeight = Math.max(mouseY - y, minSize); break;
                case 'bottom-right': newWidth = Math.max(mouseX - x, minSize); newHeight = Math.max(mouseY - y, minSize); break;
            }
            // Para evitar bugs visuais com a rotação, aplicaremos a lógica simplificada
            // que pode não ser ideal, mas evita que a imagem "salte" de forma estranha.
            obj.x = newX; obj.y = newY; obj.width = newWidth; obj.height = newHeight;
        }
